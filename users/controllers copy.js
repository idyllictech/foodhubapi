...
const addTalenter = async (req, res) => {
    ...
    pool.query(queries.checkEmailExists, [email], (error, results) => {
        ...
        //     });
            ...
            // const myval = [{valukey : encoded}];

            // console.log(myval);
            // console.log(myval[0].valukey);
...
const verifyOTP = async (req, res) => {
    ...
    try {
        ...
        if(!verification_key){
            const response={"success":"false", "message":"Verification Key not provided"}
        ...
        if(!check){
            const response={"success":"false","Details":"Check not Provided"}
            return res.status(400).json(response) 
        }
        ...
        pool.query(queries.findInstancteOTPID, [obj.otp_id], (error, results) => {
            if (error) {
                //throw error
                return res.status(400).json({message: 'Oops! Something went worng', succes: false});
            }
                ...
                if (results[0].verified !== 1) {
                    ...
                    if (dates.compare(results[0].expiration_time, currentdate)==1){
                        ...
                        if(otp === results[0].otp){
                            ...
                            [verifiedid, verifiedEmail], (error, results) => {
                                if (error) {
                                ...
                                //Send Successful OTP Registration email
                                    ...
                                    transporter.sendMail(mailOptions, function(error, response) {
                                        if (error) {
                                            console.log('Mail SMTP sending error' + error);

                                            const response={"success":"true", "message":"Activated Successfully, Please Log in."}
