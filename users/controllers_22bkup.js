    ...
    //Generate OTP
        ...
        //    const now = new Date.UTC(year, month, day, hour + 1, minute, second);
           ...
           console.log(`New Generated time: `, now);
            ...
            pool.query(queries.instanceOTP, [otp, expiration_time], async (error, results) => {
                if (error) {
                    return res.status(404).send({message: 'Oops! Somehing went wrong: ' + error});
                }                            
...
const verifyOTP = async (req, res) => {
    ...
    //try {
        ...
        if(!otp) {            
            return res.status(400).json({message: 'OTP not Provided', success: false})
          }

        if(!check){
            const response={"success":"false","message":"Check not Provided"}
            return res.status(400).json(response) 
        }
        ...
        try {
           ...
           // const response={"Status":"Failure", "Details":"Bad Request"}
            return res.status(400).json({message: 'Bad Requests', success: false});
        }
        ...
        pool.query(queries.findInstancteOTPID, [obj.otp_id], (error, results) => {
            if (error) {
                //throw error
                return res.status(400).json({message: 'Oops! Something went wrong', succes: false});
            }
                       ...
                       //Check if OTP is equal to the OTP in the DB
                        if(otp === results[0].otp){
                            ...
                            [verifiedid, verifiedEmail,verifiedEmail], (error, results) => {
                                if (error) {
                                    return res.status(400).json({message: 'All we know is something went wrong', success: false});
                                }
...
const resetPassword = async (req, res) => {
    ...
    pool.query(queries.getUserLogin, [email.toLowerCase()], async (error, results) => {
        ...
        if(error) {
            // throw error; 
            return res.status(404).send({message: error, succes: false});
        }
        ...
        pool.query(queries.updateResetPassword, [hashedPassword, verifyemail], (error, resultt) => {
            ...
            if (error) {
                // throw error
                return res.status(404).send({message: 'Oops! Something went wrong', succes: false});                
            }
            console.log(resultt);
            ...
            return res.status(200).header('Access-Control-Allow-Headers',
                     'Origin, X-Requested-With, Content-Type, Accept').json({
                    success: true, 
                    message: "Password Reset, Please Check your email.!!",
                    status: 2
                ...
                return res.status(404).send({message: 'Email address not found.', succes: false, status: 1}); 
            }
...
const deleteOTP = (req, res) => {
    ...
    pool.query(queries.getOTPByID, [id], (error, results) => {
        ...
        if (noOTPFound) {
            return res.status(404).send("OTP does not exist in the database.");
        };
